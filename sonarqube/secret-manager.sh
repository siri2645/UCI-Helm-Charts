...
# Variables
NAMESPACE="sonarsecrets"
RELEASE_NAME="sonarsecrets"
...
if [ $? -eq 0 ]; then
  ...
  kubectl create -f external-secrets.yaml
...
helm upgrade --install $RELEASE_NAME external-secrets/external-secrets --namespace $NAMESPACE --create-namespace --values external-secrets-values.yaml

if [ $? -eq 0 ]; then
  echo "external-secrets successfully installed."
 # kubectl delete validatingwebhookconfigurations externalsecret-validate
 # echo "externalsecret-validate webhook deleted successfully."
